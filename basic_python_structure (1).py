# src/__init__.py
"""
Phuzzy - Fuzzy Logic Virtual Bear Development Team

A revolutionary approach to AI agent development using fuzzy logic
for human-like reasoning and decision-making, powered by adorable bear agents.
"""

__version__ = "0.1.0"
__author__ = "Allen Partridge"
__email__ = "allen@phuzzy.dev"
__bear_mascot__ = "Phuzzy 🐻🤓"
...
"""
Base bear agent class providing core functionality for all Phuzzy bear agents.
"""
...
from ..fuzzy_logic import FuzzyBearConfidence
from ..memory import BearWorkingMemory, BearEpisodicMemory
from ..communication import BearMessageBus


class BaseBearAgent(ABC):
    """
    Base class for all Phuzzy bear agents.
    
    Provides core bear functionality:
    - Fuzzy logic bear reasoning 🧠
    - Bear memory management 💾
    - Inter-bear communication 💬
    - Bear performance monitoring 📊
    """
    
    def __init__(self, name: str, agent_type: str, bear_personality: str = "friendly"):
        self.name = name
        self.agent_type = agent_type
        self.bear_personality = bear_personality
        self.agent_id = f"{agent_type}_bear_{name}_{int(time.time())}"
        
        # Initialize core bear systems
        self.fuzzy_engine = FuzzyBearConfidence()
        self.working_memory = BearWorkingMemory(capacity=7)  # Miller's magic number (bears remember 7±2 things)
        self.episodic_memory = BearEpisodicMemory(agent_id=self.agent_id)
        self.message_bus = BearMessageBus()
        
        # Bear performance tracking
        self.start_time = datetime.now()
        self.decision_count = 0
        self.bear_energy_level = 1.0  # Bears get tired
        self.active = True
        
        # Bear logging
        self.logger = logging.getLogger(f"phuzzy.{self.agent_type}.{self.name}")
        self.logger.info(f"🐻 {self.name} the {self.bear_personality} {self.agent_type} bear is awakening...")
    
    @abstractmethod
    def process_bear_task(self, task: Dict[str, Any]) -> Dict[str, Any]:
        """Process a task using bear-specific logic."""
        pass
    
    @abstractmethod
    def get_bear_capabilities(self) -> Dict[str, float]:
        """Return bear capabilities with confidence scores."""
        pass
    
    def assess_bear_confidence(self, claim: str, evidence: Dict[str, Any]) -> Dict[str, Any]:
        """
        Assess confidence in a claim using fuzzy bear logic.
        
        Args:
            claim: The statement to evaluate (what the bear is thinking about)
            evidence: Supporting evidence with quality metrics
            
        Returns:
            Fuzzy bear confidence assessment with linguistic description
        """
        bear_assessment = self.fuzzy_engine.evaluate_like_bear(claim, evidence)
        bear_assessment['bear_wisdom'] = self._add_bear_wisdom(bear_assessment['numeric'])
        return bear_assessment
    
    def _add_bear_wisdom(self, confidence: float) -> str:
        """Add bear-specific wisdom based on confidence level."""
        if confidence <= 3:
            return "This bear suggests being extra cautious 🐻😰"
        elif confidence <= 7:
            return "This bear thinks it's worth investigating further 🐻🤔"
        else:
            return "This bear is feeling quite confident about this! 🐻😊"
    
    def remember_like_bear(self, event: Dict[str, Any], confidence: float = 1.0):
        """Store an event in bear episodic memory with confidence tracking."""
        bear_memory_entry = {
            'event': event,
            'timestamp': datetime.now().isoformat(),
            'confidence': confidence,
            'bear_id': self.agent_id,
            'bear_mood': self._get_current_bear_mood(),
            'bear_energy': self.bear_energy_level
        }
        self.episodic_memory.store_bear_memory(bear_memory_entry)
    
    def _get_current_bear_mood(self) -> str:
        """Determine current bear mood based on energy and recent decisions."""
        if self.bear_energy_level > 0.8:
            return "energetic_bear"
        elif self.bear_energy_level > 0.5:
            return "content_bear"
        elif self.bear_energy_level > 0.2:
            return "tired_bear"
        else:
            return "sleepy_bear"
    
    def send_bear_message(self, recipient: str, message_type: str, content: Dict[str, Any]):
        """Send a message to another bear agent."""
        bear_message = {
            ...
            'content': content,
            'timestamp': datetime.now().isoformat(),
            'bear_sender_mood': self._get_current_bear_mood(),
            'bear_politeness_level': 0.9  # Bears are generally polite
        }
        self.message_bus.send_bear_message(bear_message)
        self.logger.debug(f"📤 {self.name} bear sent {message_type} message to {recipient}")
    
    def get_bear_status(self) -> Dict[str, Any]:
        """Return current bear agent status and performance metrics."""
        uptime = datetime.now() - self.start_time
        
        return {
            'bear_id': self.agent_id,
            'bear_name': self.name,
            'bear_type': self.agent_type,
            'bear_personality': self.bear_personality,
            'bear_active': self.active,
            'bear_energy_level': self.bear_energy_level,
            'bear_mood': self._get_current_bear_mood(),
            'uptime_seconds': uptime.total_seconds(),
            'decision_count': self.decision_count,
            'bear_memory_usage': {
                'working_memory_items': len(self.working_memory.items),
                'episodic_memory_entries': self.episodic_memory.count_bear_memories()
            }
        } import MessageBus
